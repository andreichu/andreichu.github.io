        ...
        if (!$('html').hasClass('is-builder')){
            $(document).click(function(e){
                try {
                    ...
                    if ($(target).parents().hasClass('dev-testimonials1')) {
                        return;
                    }
                    do {
                        if (target.hash){
...
// ------ Testimonials-----------
  ...
  if ($('html').hasClass('is-builder')) {
    $(document).on('add.cards change.cards', function(event) {
      if ((!$(event.target).hasClass('dev-testimonials1')) &&
        (!$(event.target).hasClass('dev-testimonials3'))) return;

      setTimeout(function(){$(window).trigger('resize');},100);

      if ($(event.target).hasClass('dev-testimonials3')) {
      ...
      var _this = this;
      if ($(event.target).hasClass('dev-testimonials3')&&!slideEventFlag) {
        slideEventFlag =true;

        $(this).on('slide.bs.carousel', 'section.dev-testimonials3', function(event) {
          var visibleSlides = $(event.target).find('.carousel-inner').attr('data-visible');
          // Refresh all slides
          $(event.target).find('.clonedCol').remove();
          $(event.target).find('.carousel-item').each(function() {
  ...
  if (!$('html').hasClass('is-builder')) {
    ...
    if ($('section.dev-testimonials1:not(.extTestimonials5)').length > 0) {
      $(window).on('resize', function() {
        var sections = $('section.dev-testimonials1:not(.extTestimonials5)');
